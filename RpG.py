...
class RpG(loader.Module):
    ...
    strings = {
        "name": "RpG",
        "schedule_found": "<emoji document_id=5431897022456145283>📆</emoji> Расписание для 11 A на {day}:\n{lessons}"
    }
    ...
    # Фиксированное расписание для класса 11 A
    schedule = {
        "понедельник": [
            ("1", "прац.навуч.", ""),
            ("2", "прац.навуч.", ""),
    ...
    def get_day_name(self, day_num):
        ...
        return days[day_num] if 0 <= day_num < 7 else "понедельник"
    ...
    async def rpgcmd(self, message):
        ...
        if day_name not in self.schedule:
            await utils.answer(message, f"<emoji document_id=5210952531676504517>❌</emoji> Расписание на {day_name} не найдено!")
            return
