...
class UserInfoMod(loader.Module):
    """Получение информации о пользователе или канале Telegram, включая дату регистрации и данные из @funstat_obot"""

    strings = {
        "name": "UserInfo",
        "loading": "🕐 <b>Обработка данных...</b>",
        "not_chat": "🚫 <b>Это не чат!</b>",
        "unblock_bot": "❗ Разблокируйте @funstat_obot для получения дополнительной информации.",
        "timeout": "⚠️ Время ожидания ответа от @funstat_obot истекло.",
    ...
    async def process_user_info(self, user_ent, message):
        ...
        user_info += f"<b>📊 Информация из @funstat_obot:</b>\n{funstat_info}"
    ...
    async def get_funstat_info(self, user_id: int) -> str:
        """Отправка запроса в @funstat_obot и получение информации"""
        chat = "@funstat_obot"
