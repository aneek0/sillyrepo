...
# Константы
MAX_BUTTONS_PER_PAGE = 80
MAX_TOOL_ITERATIONS = 3
SUPPORTED_TEXT_MIMES = [
    'text/', 'application/json', 'application/xml', 'text/html', 
    'text/csv', 'application/javascript', 'application/x-sh', 'application/x-python'
]

# Ключевые слова для определения необходимости поиска (Gemini модели)
SEARCH_KEYWORDS = {
    "explicit": ["найди", "найти", "поиск", "поищи", "найди в интернете"],
    "time": ["сейчас", "сегодня", "недавно", "вчера", "завтра", "2024", "2025", "последние", "текущ", "актуальн"],
    "news": ["новост", "события", "происходит", "случилось"],
    "questions": ["кто", "что", "где", "когда", "почему", "как", "сколько"]
}
...
class AzuAI(loader.Module):
    ...
    async def _ask_openrouter(self, message, query, history, media_path, chat_id):
        ...
        tools = self._create_search_tool() if self.config["TAVILY_API_KEY"] else []
        payload = {"model": self.selected_models["openrouter"], "messages": messages}
        if tools:
            payload["tools"] = tools
            payload["tool_choice"] = "auto"

        try:
            answer = await self._process_tool_calling_loop(
                url, headers, payload, messages, query, message, chat_id, media_path
            )
            if answer:
    ...
    async def _prepare_onlysq_messages(self, messages, query, media_path, message):
        ...
        if media_path:
            ...
            if not query:
                await utils.answer(message, "OnlySq: Не удалось сформировать запрос (нет текста или медиа).")
                return []
            messages.append({"role": "user", "content": query})
        
        return messages

    async def _process_tool_calling_loop(self, url, headers, payload, messages, query, message, chat_id, media_path):
        """Обработать цикл tool calling для OpenRouter"""
        async with aiohttp.ClientSession() as session:
            for iteration in range(MAX_TOOL_ITERATIONS):
                async with session.post(url, json=payload, headers=headers) as response:
                    if response.status != 200:
                        error_text = await response.text()
                        await utils.answer(message, f"Ошибка от OpenRouter: Status {response.status}: {error_text[:200]}...")
                        return None
                    ...
                    if "tool_calls" in message_data and message_data["tool_calls"]:
                        ...
                        for tool_call in message_data["tool_calls"]:
                            if tool_call["function"]["name"] == "search_internet":
                                ...
                                content = f"Результаты поиска в интернете:\n\n{search_results}" if search_results else "Не удалось найти информацию в интернете."
                                messages.append({
                                    "role": "tool",
    ...
    async def _process_onlysq_completion(self, client, messages, tools, query, message, chat_id, media_path):
        """Обработать completion для OnlySq с поддержкой tool calling"""
        for iteration in range(MAX_TOOL_ITERATIONS):
            completion_kwargs = {
            ...
            if not completion.choices:
                await utils.answer(message, "⚠️ OnlySq вернул пустой ответ (нет choices).")
                return None

            message_data = completion.choices[0].message
            if not message_data:
                await utils.answer(message, "⚠️ OnlySq вернул ответ без message.")
                return None

            if message_data.tool_calls and len(message_data.tool_calls) > 0:
                assistant_message = {
                    "role": "assistant",
                    "tool_calls": [
                        {
                ...
                if message_data.content:
                    assistant_message["content"] = message_data.content
                ...
                answer = message_data.content or ""
                if not answer:
                    if iteration > 0:
                        await utils.answer(message, "⚠️ OnlySq не вернул финальный ответ после обработки tool calls.")
                    else:
                        await utils.answer(message, "⚠️ Получен пустой ответ от OnlySq.")
                    return None
                return answer
