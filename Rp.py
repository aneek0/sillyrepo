...
class Rp(loader.Module):
    ...
    async def stcmd(self, message):
        ...
        if not PDF_AVAILABLE and not PDFPLUMBER_AVAILABLE:
            await utils.answer(message, "<b>Ошибка:</b> Необходимо установить PyPDF2 или pdfplumber для работы с PDF")
            return
    ...
    async def _extract_pdf_text(self):
        ...
        try:
            ...
            # Пробуем pdfplumber (более надежный)
            if PDFPLUMBER_AVAILABLE:
                try:
                    ...
                    if not PDF_AVAILABLE:
                        raise Exception("Ошибка pdfplumber и PyPDF2 недоступен")
            
            # Если pdfplumber не сработал, пробуем PyPDF2
            if not full_text and PDF_AVAILABLE:
                try:
                    content.seek(0)
